...
InstrList* generateCode(Stm program) {
    ...
    while (current != NULL) {
        switch (current->instr.opcode) {
            ...
            case OP: {
                ...
                printf("%s := %s %s %s\n", current->instr.arg1, current->instr.arg2, op_str, current->instr.arg3);
                break;
            }
            case LABEL:
                printf("%s:\n", current->instr.arg1);
            ...
            case COND: {
                ...
                printf("if %s %s %s goto %s\n", current->instr.arg1, op_str, current->instr.arg2, current->instr.arg3);
                break;
            }
        }
